...
@section('bet')
    @php
        $banners = getContent('slider.element', false, null, true);
        ...
        <div class="banner-slider hero-slider mb-3">
            @foreach ($banners as $banner)
                <div class="banner_slide">
                    <img class="banner_image" src="{{ frontendImage('slider', @$banner->data_values->image, '1610x250') }}">
                </div>
            @endforeach
        </div>
    </div>
                            ...
                            <span class="sports-card__head">
                                <span class="sports-card__team">
                                    <span class="sports-card__team-flag">
                                        <img class="sports-card__team-flag-img" src="{{ @$game->teamOne->teamImage() }}"
                                            alt="@lang('image')">
                                    </span>
                                ...
                                <span class="sports-card__team">
                                    <span class="sports-card__team-flag">
                                        <img class="sports-card__team-flag-img" src="{{ @$game->teamTwo->teamImage() }}"
                                            alt="@lang('image')">
                                    </span>
                                    <span class="sports-card__team-name">
                                        ...
                                        @if ($more > 0)
                                            <div class="text-center mt-1">
                                                <a href="{{ route('game.markets', $game->slug) }}?more={{ $more }}"
                                                    class="text--small"> +{{ $more }} @lang('More')</a>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                        ...
                                        <div class="option-odd-list__item">
                                            <div>
                                                <button
                                                    ...
                                                    @disabled($game->bet_start_time >= now())>{{ rateData($option->odds) }} </button>
                                                <span
