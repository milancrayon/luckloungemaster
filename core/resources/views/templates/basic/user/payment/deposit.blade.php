            ...
            <div class="row justify-content-center">
                <div class="col-lg-9">
                    <form action="{{ route('user.deposit.insert') }}" method="post" class="deposit-form">
                        ...
                        <input type="hidden" name="option_identifier">
                        <div class="gateway-card">
                            <div class="row justify-content-center gy-sm-4 gy-3 mb-5">
                                <div class="col-lg-12">
                                    <div class="card">
                                        <div class="card-body p-0">
                                            <div class="payment-system-list gateway-option-list">
                                                ...
                                                @php 
                                                    $extra = json_decode($data->method->extra);
                                                    if($extra){
                                                        $total_img = sizeOf($extra->image);
                                                        $_currentOption =  mt_rand(0, ($total_img-1));
                                                        $_dimg = $extra->image[$_currentOption]; 
                                                        $_o_identifier = $extra->identifier[$_currentOption];
                                                    }else{
                                                        $_dimg = "/"; 
                                                        $_o_identifier = $extra->identifier[$_currentOption];
                                                    }
                                                @endphp
                                                    <label for="{{ titleToKey($data->name) }}" class="payment-item @if ($loop->index > 4) d-none @endif gateway-option">
                                                        ...
                                                        <input class="payment-item__radio gateway-input" id="{{ titleToKey($data->name) }}" hidden
                                                               ...
                                                               data-option="{{$_dimg}}"
                                                               data-option-identifier="{{$_o_identifier}}"
                                                               >
                                                    </label>
                                                @endforeach
                            ...
                            <div class="row justify-content-center gy-sm-4 gy-3">
                                <div class="col-lg-6">
                                    <div class="card">
                                        <div class="card-body p-0">
                                            <div>
                                                <img class="img scan-img" src="/" alt="qr code">
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        (function($) {
            ...
            function gatewayChange() {
                ...
                let option = gatewayElement.data('option'); 
                let option_identifier = gatewayElement.data('option-identifier'); 
                $('.scan-img').attr("src",option);
                $("input[name=option_identifier]").val(option_identifier);
