...
class Question extends Model {
    ...
    public function upcomingGame() {
        return $this->belongsTo(Betgame::class)->where('bet_start_time', '>=', now())->count();
    }
    ...
    public function scopeQuestionAvailable($query) {
        $query->unLocked()->resultUndeclared()->with('options.bets', 'betDetails.bet')->withWhereHas('betgame', function ($betgame) {
            $betgame->expired()->with([
                'teamOne',
                'teamTwo',
