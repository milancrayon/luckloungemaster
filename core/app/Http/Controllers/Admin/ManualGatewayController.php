...
class ManualGatewayController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $method->extra = json_encode($request->option_generator);
    ...
    public function edit($alias)
    {
        ...
        $_extra = json_decode($method->extra);

        $__extra = [];
        if ($_extra) {
            foreach ($_extra->identifier as $key => $value) {
                $_temp = array(
                    'identifier' => $_extra->identifier[$key],
                    'instruction' => $_extra->instruction[$key],
                    'image' => $_extra->image[$key],
                );
                $__extra[] = (object) $_temp;
            }
        }
        $extra = array(
            'option_data' => $__extra,
        ...
        $extra = (object) $extra;
        return view('admin.gateways.manual.edit', compact('pageTitle', 'method', 'form', 'extra'));
    }
    ...
    private function validation($request, $formProcessor)
    {
        ...
        // $optionValidation = $formProcessor->optionValidation();
        // $validation = array_merge($validation,$optionValidation['rules']);
