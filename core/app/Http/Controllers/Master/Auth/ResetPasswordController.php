...
class ResetPasswordController extends Controller
{
    public function showResetForm(Request $request, $token)
    {
        ...
        $resetToken = MasterPasswordReset::where('token', $token)->where('status', Status::ENABLE)->first();
        ...
        return view('master.auth.passwords.reset', compact('pageTitle', 'email', 'token'));
    }
    ...
    public function reset(Request $request)
    {
        ...
        $reset = MasterPasswordReset::where('token', $request->token)->orderBy('created_at', 'desc')->first();
        $master = Master::where('email', $reset->email)->first();
