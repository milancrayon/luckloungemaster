...
class PaymentController extends Controller
{
    public function deposit()
    {
        $gatewayCurrency = GatewayCurrency::whereHas('method', function ($gate) {
            $gate->select(['name', 'alias', 'extra', 'id', 'form_id'])->where('status', Status::ENABLE);
    ...
    public function depositInsert(Request $request)
    {
        ...
        if (isset($request->option_identifier)) {
            $_detail = [
                'option_identifier' => $request->option_identifier,
            ];
            $data->detail = $_detail;
        }
        $data->save();
    ...
    public function manualDepositUpdate(Request $request)
    {
        ...
        return to_route('user.deposit.view', ['id' => $data->id])->withNotify($notify);
    }

    public function view($id)
    { 
        $data = Deposit::with('gateway')->where('id', $id)->first();
        abort_if(!$data, 404); 
        $pageTitle = 'Deposit Detail'; 
        return view('Template::user.depositview', compact('data', 'pageTitle'));
